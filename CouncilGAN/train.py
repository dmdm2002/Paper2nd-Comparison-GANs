...
parser.add_argument('--configs', type=str, default='configs/glasses_folder.yaml', help='Path to the configs file.')
...
shutil.copy(opts.config, os.path.join(config_backup_folder, ('config_backup_' + str(datetime.datetime.now())[:19] + '.yaml').replace(' ', '_')))  # copy configs file to output folder
...
if config['misc']['do_telegram_report']:           
    ...
    if not os.path.exists(confidential_yaml_file_path):
        ...
        if in_.upper() != 'NO':    
            ...
            print('==== You can turn telegram report OFF from the configs.yaml file ====')
...
try:
    if config['misc']['do_telegram_report']:
        ...
        if send_telegram_config:
            try:
                telegram_bot_send_document(bot_document_path=opts.config, filename='configs.txt')
            except:
                print('telegram configs message send failed')
    while True:
