...
datasets = Loader(args.custom_path, content_tf)
data_iter = iter(data.DataLoader(
    datasets, batch_size=args.batch_size,
    sampler=InfiniteSamplerWrapper(datasets),
...
for i in tqdm(range(args.max_iter)):
    ...
    content_images, style_images, image_namge = next(data_iter)
    ...
    if i % 100 == 0:
        output_name = '{:s}/test/{:s}{:s}'.format(
                        args.save_dir, str(image_namge),".jpg"
                    )
