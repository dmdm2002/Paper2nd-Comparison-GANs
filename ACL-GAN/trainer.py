...
#        self.dis_2B = MsImageDis(hyperparameters['input_dim_a'], hyperparameters['dis'])  # discriminator 2 for domain B
    ...
    def forward(self, x_a, x_b, hyperparameters):
        ...
        if focus_lambda > 0:
            ...
            self.loss_gen_focus_A2_size = (F.relu(torch.sum(x_A2_focus - focus_upper),
                                                  inplace=True) ** 2) * focus_delta + \
            ...
            self.loss_gen_total += focus_lambda * (self.loss_gen_focus_B_size + self.loss_gen_focus_B_digit + \
                                                   ...
                                                   self.loss_gen_focus_A2_size + self.loss_gen_focus_A2_digit) / x_a.size(
                2) / x_a.size(3) / x_a.size(0) / 3
...
if __name__ == '__main__':
    ...
    parser.add_argument('--configs', type=str, default='IRIS_DATA.yaml', help='Path to the configs file.')
    opts = parser.parse_args()

    config = get_config(opts.configs)
    trainer = aclgan_Trainer(config).cuda()
    gen_AB = AdaINGen(config['input_dim_a'], config['gen']).cuda()  # auto-encoder for domain A
    gen_BA = AdaINGen(config['input_dim_a'], config['gen']).cuda()  # auto-encoder for domain B
    disc = MsImageDis(config['input_dim_a'], config['dis']).cuda()
    dis_2 = MsImageDis(config['input_dim_b'], config['dis']).cuda()
    # c_1, _ = gen_AB.encode(x_a)
    # c_2, s_2 = gen_BA.encode(x_a)
    # c_4, s_4 = gen_AB.encode(x_b)
    #
    input_img_a = torch.ones(1, 3, 224, 224).cuda()
    input_img_b = torch.ones(1, 3, 224, 224).cuda()
    flops = FlopCountAnalysis(trainer, (input_img_a, input_img_b, config))
    print(flop_count_table(flops))  # 테이블 형태로 각 연산하는 모듈마다 출력해주고, 전체도 출력해줌

    flops = FlopCountAnalysis(gen_AB, (input_img_a))
    print(flop_count_table(flops))  # 테이블 형태로 각 연산하는 모듈마다 출력해주고, 전체도 출력해줌

    flops = FlopCountAnalysis(disc, (input_img_a))
    print(flop_count_table(flops))  # 테이블 형태로 각 연산하는 모듈마다 출력해주고, 전체도 출력해줌

    flops = FlopCountAnalysis(dis_2, (input_img_b))
    print(flop_count_table(flops))  # 테이블 형태로 각 연산하는 모듈마다 출력해주고, 전체도 출력해줌